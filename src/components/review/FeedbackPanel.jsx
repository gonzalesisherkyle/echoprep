...
export function FeedbackPanel({ feedback }) {
  ...
  return (
    <Card padding="lg" as="section" className="relative overflow-hidden group border-white/5">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="grid gap-3">
            {improvementTips.map((tip, index) => (
              <li
                key={index}
                className="flex items-start gap-4 p-4 rounded-lg bg-white/2 border border-white/5 hover:border-primary/20 transition-all group/tip"
              >
